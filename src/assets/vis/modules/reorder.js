/* Version: 2.2.4 - March 15, 2024 18:32:49 */
...
(function (global, factory) {
  ...
  function gaussian() {
    ...
    gaussian.pdf = function (x) {
      x = (x - mean) / sigma;
      return (
        (science_stats_distribution_gaussianConstant * Math.exp(-0.5 * x * x)) /
        sigma
      );
    };
    ...
    function smooth(xval, yval, weights) {
      ...
      while (++iter <= robustnessIters) {
        ...
        while (++i < n) {
          ...
          var edge =
            xval[i] - xval[ileft] > xval[iright] - xval[i] ? ileft : iright; // Compute a least-squares linear fit weighted by
  ...
  function count_crossings(graph, north, south) {
    ...
    for (let i = 0; i < north.length; i++) {
      const n = invert
        ? graph
            .inEdges(north[i])
            .map((e) => [south_inv[e.target.index], e.value])
        : graph
            .outEdges(north[i])
            .map((e) => [south_inv[e.source.index], e.value]);
      n.sort(cmp_number);
  ...
  function barycenter(graph, comp, max_iter, timesteps) {
    ...
    if (comp.length < 3) {
      return [
        layer1,
        layer2,
        count_all_crossings(graph, layer1, layer2, timesteps),
      ];
    }
        ...
        for (let i = 0; i < n; i++) {
          ...
          for (let j = 0; j < n; j++) {
            if (j < i) {
              ...
              if (square) {
                d[j] =
                  distance$1(vectors[i], vectors[j]) *
                  distance$1(vectors[i], vectors[j]);
              } else {
