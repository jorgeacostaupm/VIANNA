  ...
  drawHierarchy(source, instant = false) {
    ...
    root.descendants().forEach((node) => {
      if (node.parent && node.parent.id === 0) {
        ...
        node.y = node.depth * Math.min(longestString * 20, horizontalDistance);
      }
    });
  ...
  drawNodes(source, instant = false) {
    ...
    const gnode = main
      ...
      .data(
        nodes,
        (node) =>
          node.data.name + node.data?.formula + node?.children + node?._children
      );
    ...
    function colorNode(node) {
      ...
      if (node.data.type === "aggregation") {
        if (node.data?.formula === "")
          return node?.children?.length > 0 ? "grey" : "black";
        else return node?.children?.length > 0 ? "#aed1b4" : "#1b7a17";
      } else if (node.data.type === "attribute") {
        if (node?._children?.length > 0) return "darkblue";
        return node?.children?.length > 0 ? "lightblue" : "#1677ff";
      }
    }
