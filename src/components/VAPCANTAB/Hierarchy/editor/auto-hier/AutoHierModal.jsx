...
const AutoHierModal = ({ setOpen, isModalOpen, setIsModalOpen }) => {
  ...
  const attributes = useSelector(selectAttributes).filter((m) => m.type === 'attribute');

  if (worker == null) {
    console.log('meta URL', import.meta.env.BASE_URL);
    fetchTestData();
  ...
  return (
    <>
      ...
      >
        <span style={{ color: '#4b5563', opacity: 0.6, marginBottom: '0.25rem' }}>
          Aggregation operations should be configured manually
        </span>
        <div
          style={{ height: '100%', margin: '0.5rem 0', display: 'flex', flexDirection: 'column' }}
        >
          <div style={{ margin: '0.5rem 0' }}>
            ...
            {attributes.length === 0 && (
              <span style={{ color: '#dc2626', marginLeft: '0.75rem' }}>
                There are not <strong style={{ color: '#dc2626' }}>Attributes</strong> charged.{' '}
              </span>
            )}
          </div>
          {attributes.length > 0 && process === 'npl-cluster' && isModalOpen && (
            <ClusteringProcess
              close={handleCancel}
        ...
        >
          {progress.map((p) => {
            return <Progress key={p.file} text={p.file} percentage={p.progress} />;
          })}
        </div>
      </Modal>
    </>
