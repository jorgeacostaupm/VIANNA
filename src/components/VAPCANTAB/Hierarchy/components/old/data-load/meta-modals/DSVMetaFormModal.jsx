...
import { addDescriptionsFromCSV } from "@/components/VAPUtils/features/metadata/metaSlice";

import LoadingDataSpinner from "../common/LoadingDataSpinner";

const CSVConfigurationStep = ({ data, prev, closeFn, next, columns}) => {

    const { publish } = pubsub;
    const dispatch = useDispatch();

    data.opts.attributes = columns[0]
    data.opts.descriptions = columns[1]

    const onSubmit = (values) => {
        publish("hidePreviewEvent", {})
        dispatch(addDescriptionsFromCSV(values));
        next();
    };
    ...
    const validationSchema = yup.object().shape({
        ...
        opts: yup
            ...
            .shape({
                ...
                attributes: yup
                    .string()
                    .required(
                        "Debes seleccionar la columna de atributos"
                    ),
                descriptions: yup
                    .string()
                    .required(
        ...
        name: 'test-match-cols',
        test: (values) => {
            if (values.opts.descriptions === values.opts.attributes) {
                return this.createError({
                    path: 'opts.descriptions',
                    message: 'La columna de Atributos y Descripciones no puede ser la misma',
                })
            }
            return true;
        }
    ...
    return (
        ...
        >
            {({
                ...
                }, [values.opts.delimiter, values.opts.encoding, values.opts.decimal]);
                ...
                return (
                    <Form className="w-full flex-col flex">
                        ...
                        <div className="grid grid-cols-3 items-center w-full mt-6 gap-y-8 gap-x-4 ml-8">
                            ...
                            <div className="flex items-center justify-start gap-4 p-4">
                                ...
                                >
                                    {columns.map((c) => {
                                        return (<option key={"1-" + c} value={c}>{c}</option>)
                                    })}
                                </Field>
                            </div>
                            ...
                            <div className="flex items-center justify-start gap-4 p-4">
                                ...
                                >
                                    {columns.map((c) => {
                                        return (<option key={"2-" + c} value={c}>{c}</option>)
                                    })}

                                </Field>
                            </div>
                            <span className="text-red-500 text-lg">{errors && errors.opts && errors.opts.descriptions}</span>
                            </div>
...
const DSVMetaFormModal = ({ setSource }) => {
    const { publish } = pubsub;
...
export default DSVMetaFormModal;
