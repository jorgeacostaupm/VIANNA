...
const OptionMenu = ({ start, openImportModal, openAutoModal }) => {
  ...
  const handleHierDataDiff = () => {
    const hierarchyVars = hierarchy
      .filter((d) => d.type === "attribute")
      .map((d) => d.name);
    const datasetVars = getDataframeVariables();

    const missing = datasetVars.filter(
      (varName) => !hierarchyVars.includes(varName)
    );
    const extra = hierarchyVars.filter(
      (varName) => !datasetVars.includes(varName)
    );
  ...
  const addMissingNodes = () => {
    ...
    publish("notification", {
      message: isMissingVars
        ? "Missing variables added"
        : "There are no missing variables",
      description: isMissingVars
        ? `Nº Missing variables added: ${missingVars.length}`
        : "",
      placement: "topRight",
  ...
  const removeExtraNodes = () => {
    ...
    publish("notification", {
      ...
      description: isExtraVars
        ? `Nº Extra variables removed: ${extraNodeIds.length}`
        : "",
      placement: "topRight",
