...
import { hierarchySelector, setFullMeta } from "@/features/metadata/metaSlice";
...
function generateFileName(baseName = "data") {
...
const ExportTab = ({ metadata }) => {
  ...
  return (
    ...
    >
      ...
      <Button
        type={"primary"}
        onClick={() => saveData2CSV(allData, "CANTAB_all")}
      >
        Export All
      </Button>
      ...
      <Button
        type={"primary"}
        onClick={() => saveData2CSV(selectionData, "CANTAB_selection")}
      >
        Export Selection
      </Button>
    </div>
...
const fileValidation = (metadata, content) => {
  const attributes = metadata
    ...
    .map((n) => n.name);
  const contentAttributes = content
    .filter((n) => n.type === "attribute")
    .map((n) => n.name);
  return (
...
const ImportTab = ({ setOpen, metadata }) => {
  ...
  return (
    ...
    >
      <div style={{ width: "100%" }}>
        <h2
          style={{ fontWeight: "bold", fontSize: "1.125rem", color: "#1677ff" }}
        >
          Data:
        </h2>
      </div>
      <div style={{ width: "50%", margin: "1rem 0" }}></div>
...
const PerModal = ({ setOpen, isModalOpen, setIsModalOpen }) => {
  const metadata = useSelector(hierarchySelector);
