...
export const CreateToMeta = createAsyncThunk(
  ...
  async (payload, thunkAPI) => {
    ...
    return new Promise(async (resolve, reject) => {
      const allNodes = thunkAPI.getState().metadata.attributes.filter((n) => n.type == 'root');
      const attributes = allNodes.filter((n) => n.type === 'attribute').map((n) => n.name);

      const newAggregations = payload.filter(
        (m) => (m.type == 'aggregation') & (attributes.find((n) => n == m.name) == null)
      );

      const oldAggregations = allNodes.filter((m) => m.type == 'aggregation').map((n) => n.name);

      await thunkAPI.dispatch(removeBatch({ cols: oldAggregations })).then((res) => {
        thunkAPI.dispatch(generateAggregationBatch({ cols: newAggregations }));
      });
...
// this may be moed to a sync reducer like a normal one or a prepared reducer
// takes attribute types and generates nodes, maybe change name
