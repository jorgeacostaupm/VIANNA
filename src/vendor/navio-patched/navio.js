...
function navio(selection, _h) {
  ...
  let nv = this || {},
    ...
    dSortHistoryByLevel = [], // keeps sort replay history per level
    dBrushes = [],
  ...
  function sortLevelBySpec(levelToUpdate, sortSpec, _dataIs) {
    if (!sortSpec || sortSpec.attrib === undefined) return false;

    _dataIs = _dataIs !== undefined ? _dataIs : dataIs;
    if (!_dataIs.hasOwnProperty(levelToUpdate)) return false;

    let before = performance.now();

    _dataIs[levelToUpdate].sort(function (a, b) {
      return sortSpec.desc
        ? d3DescendingNull(
            getAttrib(data[a], sortSpec.attrib),
            getAttrib(data[b], sortSpec.attrib)
          )
        : d3AscendingNull(
            getAttrib(data[a], sortSpec.attrib),
            getAttrib(data[b], sortSpec.attrib)
  ...
  function normalizeSortHistoryAction(action) {
    if (!action) return null;

    const attr =
      action.attrib !== undefined
        ? action.attrib
        : getAttribByName(action.attribName);
    if (attr === undefined) return null;

    return {
      attrib: attr,
      attribName: getAttribName(attr),
      desc: !!action.desc,
    };
  }

  function pushSortHistory(level, action) {
    const normalized = normalizeSortHistoryAction(action);
    if (!normalized) return false;

    if (!Array.isArray(dSortHistoryByLevel[level])) {
      dSortHistoryByLevel[level] = [];
    }
    dSortHistoryByLevel[level].push({
      attribName: normalized.attribName,
      desc: normalized.desc,
    });
    dSortBy[level] = { attrib: normalized.attrib, desc: normalized.desc };
    return true;
  }

  function applySortHistoryToLevel(levelToUpdate, _dataIs) {
    _dataIs = _dataIs !== undefined ? _dataIs : dataIs;
    const history = Array.isArray(dSortHistoryByLevel[levelToUpdate])
      ? dSortHistoryByLevel[levelToUpdate]
      : [];
    ...
    history.forEach((action) => {
      ...
      const didSort = sortLevelBySpec(
        levelToUpdate,
        { attrib: normalized.attrib, desc: normalized.desc },
        _dataIs
      );
      if (didSort) {
        applied = true;
        lastSortSpec = { attrib: normalized.attrib, desc: normalized.desc };
      }
    });

    if (applied && lastSortSpec) {
      dSortBy[levelToUpdate] = lastSortSpec;
      return true;
    }
    ...
    return sortLevelBySpec(levelToUpdate, dSortBy[levelToUpdate], _dataIs);
  }

  function updateSorting(levelToUpdate, _dataIs) {
    applySortHistoryToLevel(levelToUpdate, _dataIs);
  }
  ...
  function onSortLevel(event, d) {
    ...
    pushSortHistory(d.level, nextSort);
  ...
  function getAttribByName(attrName) {
    if (attrName === undefined || attrName === null) return undefined;

    if (dAttribs.has(attrName)) {
      return dAttribs.get(attrName);
    }

    for (let attr of attribsOrdered) {
      if (getAttribName(attr) === attrName) {
        return attr;
      }
    }
  ...
  function serializeFilter(filter, level) {
    ...
    if (filter.itemAttrName !== undefined) {
      spec.itemAttrName = filter.itemAttrName;
    }
    if (filter.firstId !== undefined) {
      spec.firstId = filter.firstId;
    }
    if (filter.lastId !== undefined) {
      spec.lastId = filter.lastId;
    }
    if (filter.selId !== undefined) {
      spec.selId = filter.selId;
    }
  ...
  function buildFilterFromSpec(spec, fallbackLevel = 0) {
    if (!spec?.type) return null;

    const level = Number.isInteger(spec.level) ? spec.level : fallbackLevel;
    const itemAttr = getAttribByName(spec.itemAttrName);
    if (itemAttr === undefined) return null;

    if (spec.type === "range" || spec.type === "negativeRange") {
      const first = dData.get(spec.firstId);
      const last = dData.get(spec.lastId);
      if (!first || !last) return null;

      const Ctor =
        spec.type === "negativeRange" ? FilterByRangeNegative : FilterByRange;
      return new Ctor({
        first,
        last,
        level,
    ...
    if (spec.type === "value" || spec.type === "negativeValue") {
      let sel = dData.get(spec.selId);
      if (!sel && spec.value !== undefined) {
        sel = data.find((row) => getAttrib(row, itemAttr) === spec.value);
      }
      if (!sel) return null;

      const Ctor =
        spec.type === "negativeValue" ? FilterByValueDifferent : FilterByValue;
      return new Ctor({
        sel,
        itemAttr,
        getAttrib,
  ...
  function applyAttribOrderByNames(attribOrderNames = []) {
    if (!Array.isArray(attribOrderNames) || attribOrderNames.length === 0) {
      return false;
    }

    const currentOrderByName = attribsOrdered.map((attr) => getAttribName(attr));
    const orderedNameSet = new Set(attribOrderNames);
    const attrByName = new Map(
      attribsOrdered.map((attr) => [getAttribName(attr), attr]),
    );

    const nextOrder = [];
    attribOrderNames.forEach((attrName) => {
    ...
    attribsOrdered.forEach((attr) => {
      const attrName = getAttribName(attr);
      if (!orderedNameSet.has(attrName)) {
        nextOrder.push(attr);
      }
    });

    const nextOrderByName = nextOrder.map((attr) => getAttribName(attr));
    const changed =
      nextOrderByName.length === currentOrderByName.length &&
      nextOrderByName.some((name, index) => name !== currentOrderByName[index]);

    if (changed) {
      attribsOrdered = nextOrder;
    }

    return changed;
  }

  function applySortStateToVisibleLevels() {
    let applied = false;

    for (let level = 0; level < dataIs.length; level++) {
      if (!dSortBy.hasOwnProperty(level)) continue;
      if (!Array.isArray(dataIs[level])) continue;

      updateSorting(level, dataIs);
      applied = true;
    }
  ...
  function applyFiltersAndUpdate(fromLevel, opts = {}) {
    const recomputeBrushes =
      opts.recomputeBrushes !== undefined ? opts.recomputeBrushes : false;
  ...
  function updateBrushes(d, level, recomputeBrushes = false) {
    ...
    if (recomputeBrushes) {
      const brushSelection = getBrushSelectionForLevel(level);
      d3.select(this)
        .selectAll(".brush")
        .call(dBrushes[level].move, brushSelection);
    }
  ...
  function getBrushSelectionForLevel(level) {
    const levelFilters = Array.isArray(filtersByLevel[level])
      ? filtersByLevel[level]
      : [];
    const rangeFilter = [...levelFilters]
      .reverse()
      .find((f) => f?.type === "range" || f?.type === "negativeRange");

    if (!rangeFilter) return null;

    const first = dData.get(rangeFilter.firstId);
    const last = dData.get(rangeFilter.lastId);
    if (!first || !last) return null;

    const yFirst = resolveItemY(level, first);
    const yLast = resolveItemY(level, last);
    if (yFirst == null || yLast == null) return null;

    const top = Math.min(yFirst, yLast);
    const bottom = Math.max(yFirst, yLast) + yScales[level].bandwidth();
    return [top, bottom];
  }

  function resolveItemY(level, item) {
    const direct = yScales[level](item[id]);
    if (direct != null) return direct;

    const itemIndexOnLevel = item?.__i?.[level];
    const itemsPerpixel = dataIs[level]?.itemsPerpixel || 1;
    const representatives = dataIs[level]?.representatives;
    if (
      itemIndexOnLevel == null ||
    ...
    const representativePos = Math.max(
      0,
      Math.min(
        representatives.length - 1,
        Math.floor(itemIndexOnLevel / itemsPerpixel),
      ),
    );
    const representativeIndex = representatives[representativePos];
    const representative = data[representativeIndex];
    if (!representative) return null;

    const fallback = yScales[level](representative[id]);
  ...
  function drawBrushes(recomputeBrushes) {
    ...
    if (recomputeBrushes) {
      levelOverlayEnter
        .merge(levelOverlay)
        .each(function (d, i) {
          updateBrushes.call(this, d, i, true);
        });
    } else {
      levelOverlayEnter.each(function (d, i) {
        updateBrushes.call(this, d, i, false);
      });
    }
  ...
  function deleteSubsequentLevels(_level, _dataIs, opts) {
    ...
    dSortBy.splice(level);
    dSortHistoryByLevel.splice(level);
  ...
  nv.sortBy = function (_attrib, _desc = false, _level = undefined) {
    ...
    if (_attrib !== undefined) {
      ...
      pushSortHistory(level, nextSort);
      updateSorting(level);
      return nv.update();
    } else {
    ...
    return {
      ...
      sortHistoryByLevel: dSortHistoryByLevel.map((levelHistory) =>
        Array.isArray(levelHistory)
          ? levelHistory
              .map((action) => normalizeSortHistoryAction(action))
              .filter(Boolean)
              .map((action) => ({
                attribName: action.attribName,
                desc: action.desc,
              }))
          : []
      ),
      filtersByLevel: filtersByLevel.map((levelFilters, level) =>
        Array.isArray(levelFilters)
          ? levelFilters
              .map((filter) => serializeFilter(filter, level))
              .filter(Boolean)
          : []
      ),
    };
  ...
  nv.importUiState = function (state, opts = {}) {
    ...
    const restoreSort =
      opts.restoreSort !== undefined ? opts.restoreSort : true;
    const restoreAttribOrder =
      opts.restoreAttribOrder !== undefined ? opts.restoreAttribOrder : true;
    ...
    if (typeof state.nestedFilters === "boolean") {
      nv.nestedFilters = state.nestedFilters;
    }

    const didRestoreAttribOrder =
      restoreAttribOrder && Array.isArray(state.attribOrder)
        ? applyAttribOrderByNames(state.attribOrder)
        : false;

    if (restoreSort && Array.isArray(state.sortHistoryByLevel)) {
      state.sortHistoryByLevel.forEach((historyActions, level) => {
        if (!Array.isArray(historyActions)) return;

        historyActions.forEach((action) => {
          pushSortHistory(level, action);
        });
      });
    }
    ...
    if (
      restoreSort &&
      dSortHistoryByLevel.every((history) => !Array.isArray(history) || history.length === 0) &&
      Array.isArray(state.sortBy)
    ) {
      state.sortBy.forEach((sortItem, level) => {
        if (!sortItem) return;
        pushSortHistory(level, {
          attribName: sortItem.attribName,
          desc: !!sortItem.desc,
        });
      });
    }

    const didApplyBaseSort = restoreSort ? applySortStateToVisibleLevels() : false;

    let hasAnyFilter = false;
    if (Array.isArray(state.filtersByLevel)) {
      state.filtersByLevel.forEach((levelFilters, level) => {
        if (!Array.isArray(levelFilters)) return;
        const hydrated = levelFilters
          .map((spec) => buildFilterFromSpec(spec, level))
          .filter(Boolean);

        filtersByLevel[level] = hydrated;
        hasAnyFilter = hasAnyFilter || hydrated.length > 0;
      });
    }

    if (!shouldApply) return nv;

    if (hasAnyFilter) {
      applyFiltersAndUpdate(0, { recomputeBrushes: true });
      return nv;
    }

    const hasSortState =
      (restoreSort && Array.isArray(state.sortBy) && state.sortBy.some(Boolean)) ||
      didApplyBaseSort;

    if (didRestoreAttribOrder || hasSortState) {
      nv.hardUpdate({ shouldUpdateColorDomains: false, recomputeBrushes: true });
    }
  ...
  nv.exportState = function () {
    return nv.exportUiState();
  };

  nv.importState = function (state, opts = {}) {
    return nv.importUiState(state, opts);
  };
